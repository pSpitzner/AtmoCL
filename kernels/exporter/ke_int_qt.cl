__kernel void ke_int_qt_kernel_main(__private parameters par,
                                    __private uint ref,
                                    __private uint dim,
                                    ...
                                    __write_only image3d_t b_target)
{
  ...
  // vertical profile for total water mixing ratio
  ...
  if      (dim == 0) {
    for (int x = 0; x < par.sx; x++) {
      ...
      q_temp = (st.rho_d != 0.0f ? (st.rho_l+st.rho_v)/st.rho_d : 0.0f);
      qt = ((float)(x)*qt + q_temp)/(float)(x+1);
    }
  }
  // XZ
  else if (dim == 1) {
    for (int y = 0; y < par.sy; y++) {
      ...
      st = init_state_with_ice(&par, &c, &c_ice);
      q_temp = (st.rho_d != 0.0f ? (st.rho_l+st.rho_v)/st.rho_d : 0.0f);
      qt = ((float)(y)*qt + q_temp)/(float)(y+1);
    }
  }
