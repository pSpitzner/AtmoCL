...
//  Tropos
//
//  Created by inDa on 17/06/15.
//
//

#include "clcontext.h"
// need additional header files to access class functions (e.g. delete)
#include "clbuffer.h"
#include "clkernel.h"

clcontext::clcontext(cllogger *loggern, int device_nr, bool profiling_enabled)
{
  logger = loggern;
  ...
  cl_platform_id scope_platform = NULL;
  cl_platform_id* platform_list;
  cl_uint num_platforms;

  logger->log(0, "Available CL Devices (choose via '-dev i')\n\n");

  ret = clGetPlatformIDs(0, NULL, &num_platforms);
  platform_list = new cl_platform_id[num_platforms];
  ret = clGetPlatformIDs (num_platforms, platform_list, NULL);

  for (cl_uint i = 0; i < num_platforms; ++i) {
    s_platform = "";

    // select platform
    scope_platform = platform_list[i];

    ret = clGetPlatformInfo(scope_platform, CL_PLATFORM_NAME, value_size, value, NULL);
    s_platform += value;
    s_platform +=  " - ";

    ret = clGetPlatformInfo(scope_platform, CL_PLATFORM_VERSION, value_size, value, NULL);
    s_platform += value;

    cl_uint device_count;
    cl_device_id *platform_devices;

    ret = clGetDeviceIDs (scope_platform, CL_DEVICE_TYPE_ALL, 0, NULL, &device_count);
    platform_devices = new cl_device_id[device_count];
    ret = clGetDeviceIDs (scope_platform, CL_DEVICE_TYPE_ALL, device_count, platform_devices, &device_count);

    for (unsigned int j = 0; j < device_count; ++j )  {
      int this_device_nr =  i*num_platforms+j;
      clGetDeviceInfo(platform_devices[j], CL_DEVICE_NAME, value_size, value, NULL);
      logger->log(0, "%d\t%s\n\t%s\n\n", this_device_nr, value, s_platform.c_str());
      if (this_device_nr == d) {
        device_id = platform_devices[j];
      }
    }
...
void clcontext::finish()
{
  ...
  logger->log(1, ret, "clFinish\n");
}

void clcontext::cl_device_info(cl_device_id device) {
  /* Copyright 1993-2009 NVIDIA Corporation.  All rights reserved.
     Modified by Mark Zwolinski, December 2009
     Modified by Robert McGibbon, August 2013
     https://github.com/miguelao/clDeviceQuery
  */
...
void clcontext::full_info() {
  ...
  uname(&unameData);
  std::string sProfileString = "OS = ";
  sProfileString += unameData.sysname;
  sProfileString += " ";
  sProfileString += unameData.release;
  sProfileString += "\nPlatform Name = ";
...
unsigned long clcontext::used_memory() {
  unsigned long mem = 0;
